...
awg genkey \
  ...
  | awg pubkey > "keys/${CLIENT_NAME}_pub"
awg genpsk | tee "keys/${CLIENT_NAME}_psk" &> /dev/null
...
if [[ -f /etc/pivpn/hosts.amneziawg ]]; then
  echo "${UNUSED_IPV4_DOT} ${CLIENT_NAME}.pivpn" \
    | tee -a /etc/pivpn/hosts.amneziawg > /dev/null
...
if [[ "${PLAT}" == 'Alpine' ]]; then
  if rc-service awg-quick restart; then
